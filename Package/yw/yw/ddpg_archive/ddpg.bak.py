...
from yw.ddpg_main.actor_critic import Actor, Critic
...
class DDPG(object):
    def __init__(
        ...
        **kwargs
        ...
        self.main_actor = Actor(
            self.dimu, name="main_actor", num_layers=self.layers, num_hidden=self.hidden, activation=tf.nn.relu
        )
        self.main_critic = Critic(
            name="main_critic", num_layers=self.layers, num_hidden=self.hidden, activation=tf.nn.relu
        )
        self.target_actor = Actor(
            self.dimu, name="target_actor", num_layers=self.layers, num_hidden=self.hidden, activation=tf.nn.relu
        )
        self.target_critic = Critic(
            name="target_critic", num_layers=self.layers, num_hidden=self.hidden, activation=tf.nn.relu
        )

        # Create networks and core TF parts that are shared across setup parts.
        self.actor_tf = self.main_actor(normalized_o)
        self.critic_tf = self.main_critic(normalized_o, normalized_u)
        self.critic_with_actor_tf = self.main_critic(normalized_o, self.actor_tf)
        Q_obs1 = self.target_critic(normalized_o_2, self.target_actor(normalized_o_2))
        self.target_Q = tf.clip_by_value(self.inputs["r"] + tf.pow(self.gamma, self.inputs["n"]) * Q_obs1, -self.clip_return, self.clip_return)

        logger.info("setting up actor optimizer")
        self.actor_loss = -tf.reduce_mean(self.critic_with_actor_tf) + self.action_l2 * tf.reduce_mean(
            tf.square(self.actor_tf)
        )
        self.actor_grads = flatgrad(self.actor_loss, self.main_actor.trainable_vars, clip_norm=self.clip_norm)
        self.actor_optimizer = MpiAdam(var_list=self.main_actor.trainable_vars, beta1=0.9, beta2=0.999, epsilon=1e-08)
        ...
        if self.demo_critic in ["rb"]:
            ...
            # rl loss
            self.critic_loss = tf.reduce_mean(tf.boolean_mask(tf.square(self.critic_tf - self.target_Q), rl_mask))
            # demo loss
            self.demo_critic_loss = tf.reduce_mean(tf.boolean_mask(tf.square(self.critic_tf - self.target_Q), demo_mask))
        else:
            self.critic_loss = tf.reduce_mean(tf.square(self.critic_tf - self.target_Q))

        self.critic_grads = flatgrad(self.critic_loss, self.main_critic.trainable_vars, clip_norm=self.clip_norm)
        self.critic_optimizer = MpiAdam(
            var_list=self.main_critic.trainable_vars, beta1=0.9, beta2=0.999, epsilon=1e-08
        )
        if self.demo_critic in ["rb"]:
            self.demo_critic_grads = flatgrad(self.demo_critic_loss, self.main_critic.trainable_vars, clip_norm=self.clip_norm)
            self.demo_critic_optimizer = MpiAdam(
                var_list=self.main_critic.trainable_vars, beta1=0.9, beta2=0.999, epsilon=1e-08
            )

        # Polyak averaging
        self.main_vars = self.main_critic.vars + self.main_actor.vars
        self.target_vars = self.target_critic.vars + self.target_actor.vars
    ...
    def get_actions(self, o, ag, g, noise_eps=0.0, random_eps=0.0, use_target_net=False, compute_Q=False):
        ...
        # values to compute
        vals = [self.actor_tf, self.critic_with_actor_tf]
        # feed
