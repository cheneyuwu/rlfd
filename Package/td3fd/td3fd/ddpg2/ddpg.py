...
class DDPG(object):
    ...
    def initialize_target_net(self):
        ...
        list(map(hard_copy_func, zip(self.target_actor.weights, self.main_actor.weights)))
        list(map(hard_copy_func, zip(self.target_critic.weights, self.main_critic.weights)))
        if self.twin_delayed:
            list(map(hard_copy_func, zip(self.target_critic_twin.weights, self.main_critic_twin.weights)))

    def update_target_net(self):
        soft_copy_func = lambda v: v[0].assign(self.polyak * v[0] + (1.0 - self.polyak) * v[1])
        list(map(soft_copy_func, zip(self.target_actor.weights, self.main_actor.weights)))
        list(map(soft_copy_func, zip(self.target_critic.weights, self.main_critic.weights)))
        if self.twin_delayed:
            list(map(soft_copy_func, zip(self.target_critic_twin.weights, self.main_critic_twin.weights)))
    ...
    def _create_network(self):
        ...
        if self.twin_delayed:
            self.main_critic_twin = Critic(self.dimo, self.dimg, self.dimu, self.max_u, self.layer_sizes)
            self.target_critic_twin = Critic(self.dimo, self.dimg, self.dimu, self.max_u, self.layer_sizes)
        # create weights
