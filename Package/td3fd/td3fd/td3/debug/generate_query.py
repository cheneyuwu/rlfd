...
def query(result, save):
    ...
    for loss_mode in ["surface_q_only", "surface_p_only", "surface_p_plus_q"]:
        ...
        g_tc = torch.tensor(g_r, dtype=torch.float).to(device)
        u_tc = torch.tensor(u_r, dtype=torch.float).to(device)

        # Normalize inputs (do not normalize inputs for gym/mujoco envs)
        norm_o_tc = policy.o_stats.normalize(o_tc)
        norm_g_tc = policy.g_stats.normalize(g_tc)

        q = policy.main_critic(o=norm_o_tc, g=norm_g_tc, u=u_tc).cpu().data.numpy()
        p = 0.0
        if policy.shaping != None:
            p = policy.shaping.potential(o_tc, g_tc, u_tc).cpu().data.numpy()

        if loss_mode == "surface_q_only":
            ret = q
        elif loss_mode == "surface_p_only" and policy.shaping != None:
            ret = p
        elif loss_mode == "surface_p_plus_q" and policy.shaping != None:
            ret = q + p
        else:
            continue
