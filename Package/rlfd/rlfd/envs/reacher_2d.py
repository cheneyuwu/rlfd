...
class Reacher:
  ...
  def __init__(self, order=2, sparse=False, block=False, seed=0):
    ...
    self.workspace = self.Block((-self.boundary, -self.boundary),
                                2 * self.boundary, 2 * self.boundary)
    self.blocks = []
    if block == True:
      # add an obstacle
    ...
    self.action_space = spaces.Box(-1.0 * self.max_u,
                                   1.0 * self.max_u,
                                   shape=(2,),
                                   dtype="float32")
  ...
  class Block:
    ...
    def plot(self, ax):
      rect = patches.Rectangle(self.start,
                               self.width,
                               self.height,
                               linewidth=1,
                               edgecolor="r",
    ...
    def inside(self, point):
      return (point[0] > self.start[0] and
              point[0] < self.start[0] + self.width and
              point[1] > self.start[1] and
              point[1] < self.start[1] + self.height)

    def outside(self, point):
      return (point[0] <= self.start[0] or
              point[0] >= self.start[0] + self.width or
              point[1] <= self.start[1] or
              point[1] >= self.start[1] + self.height)
  ...
  def render(self):
    ...
    ax.plot(self.history["position"][-1][0],
            self.history["position"][-1][1],
            "o",
            color="r",
            label="current position")
    ax.plot(self.history["goal"][-1][0],
            self.history["goal"][-1][1],
            "o",
            color="g",
            label="goal position")
  ...
  def step(self, action):
    ...
    if self.order == 2:
      ...
      if all([block.outside(new_curr_pos) for block in self.blocks
             ]) and self.workspace.inside(new_curr_pos):
  ...
  def _get_state(self):
    obs = np.concatenate(
        (self.speed, self.curr_pos)) if self.order == 2 else self.curr_pos
    g = self.goal
    ag = self.curr_pos
