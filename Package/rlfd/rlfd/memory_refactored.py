...
class ReplayBuffer(object, metaclass=abc.ABCMeta):
  ...
  def __init__(self, buffer_shapes, size):
    ...
    # buffer
    self.buffers = {
        key: np.empty([self.size, *shape], dtype=np.float32)
  ...
  def sample(self,
    ...
    if return_iterable:
      ...
      return self._sample_iterable(batch_size, shuffle, include_partial_batch)
  ...
  @abc.abstractmethod
  def _sample_iterable(self, batch_size, shuffle, include_partial_batch):
    """return a iterable from sampler"""
...
class RingReplayBuffer(ReplayBuffer):
  ...
  def sample(self, batch_size=-1):
    """
        ...
        """
    idxs = np.random.randint(0, self.current_size,
                             batch_size) if batch_size >= 0 else np.arange(
                                 self.current_size)
    batch_size = batch_size if batch_size >= 0 else self.current_size

    transitions = {
        key: self.buffers[key][idxs].copy() for key in self.buffers.keys()
    }
    transitions = {
    ...
    assert all(
        [transitions[k].shape[0] == batch_size for k in transitions.keys()])
  ...
  def _get_storage_idx(self, inc):
    ...
    if self._pointer + inc <= self.size:
      idx = np.arange(self._pointer, self._pointer + inc)
      self._pointer += inc
    else:
      overflow = inc - (self.size - self._pointer)
      idx_a = np.arange(self._pointer, self.size)
...
class UniformReplayBuffer(ReplayBuffer):
  ...
  def _sample_iterable(self, batch_size, shuffle, include_partial_batch):
    ...
    inds = np.arange(transitions["o"].shape[0])
    if shuffle:
      np.random.shuffle(inds)
...
if __name__ == "__main__":
  o = np.random.uniform(size=(2, 10, 2))
  print(o.shape)
