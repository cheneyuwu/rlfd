...
from yw.ddpg.util import flatten_grads, import_function, reshape_for_broadcasting, store_args, transitions_in_episode_batch
from yw.common.mpi_adam import MpiAdam
...
class DDPG(object):
    ...
    def get_actions(self, o, ag, g, noise_eps=0., random_eps=0., use_target_net=False,
        ...
        u += np.random.binomial(1, random_eps, u.shape[0]).reshape(-1,
                                                                   1) * (self._random_action(u.shape[0]) - u)  # eps-greedy
        if u.shape[0] == 1:
            u = u[0]
        u = u.copy()
        ret[0] = u
    ...
    def _create_network(self, reuse=False):
        ...
        self.update_target_net_op = list(map(lambda v: v[0].assign(
            self.polyak * v[0] + (1. - self.polyak) * v[1]), zip(self.target_vars, self.main_vars)))
