...
from yw.ddpg_no_goal.ddpg import DDPG
from yw.ddpg_no_goal.rollout import RolloutWorker
from yw.ddpg_no_goal.her import make_sample_transitions

DEFAULT_PARAMS = {
    # env
    "max_u": 2.0,  # max absolute value of actions on different coordinates
    # ddpg
    "layers": 2,  # number of layers in the critic/actor networks
    "hidden": 64, # 256,  # number of neurons in each hidden layers
...
def configure_dims(params):  # this is how the cached info is used for.
    ...
    dims = {"o": obs.shape[0], "u": env.action_space.shape[0]}
...
def config_demo(params, policy):
    ...
    for name in ["T", "dims", "noise_eps", "random_eps"]:
