...
#SBATCH --ntasks=80                       # number of MPI processes
#SBATCH --mem-per-cpu=6G                  # memory per node
#SBATCH --time=00-16:00:00                # time format: day-hour:min:sec
#SBATCH --output=noshaping-%N-%j.out      # %N for node name, %j for jobID
