...
class Policy(object):
  ...
  def __init__(self,
               ...
               dimu,
               get_action,
               process_observation=lambda o, g: (o, g),
  ...
  def __call__(self, o, g):
    ...
    batch_g = g.reshape((batch_o.shape[0], *self._dimg))

    o_tf = tf.convert_to_tensor(batch_o, dtype=tf.float32)
    g_tf = tf.convert_to_tensor(batch_g, dtype=tf.float32)

    u = self._call_graph(o_tf, g_tf).numpy()
  ...
  @tf.function
  def _call_graph(self, o, g):
    """TF graph to compute the output."""
    o, g = self._process_observation(o, g)
    u = self._get_action(o, g)
...
class GaussianPolicy(Policy):
  ...
  def __init__(self,
               ...
               process_observation=lambda o, g: (o, g),
               process_action=lambda u: u):
    super().__init__(dimo, dimg, dimu, self._get_action, process_observation,
                     process_action)
...
class EpsilonGreedyPolicy(Policy):
  ...
  def _get_action(self, o, g):
    ...
    u_rand = self._random_action(o, g)
...
class RandomPolicy(Policy):
  ...
  def __init__(self, dimo, dimg, dimu, max_u):
    super().__init__(dimo, dimg, dimu, self._get_action)
    self._max_u = max_u
