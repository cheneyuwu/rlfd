...
def configure_ddpg(params, reuse=False, use_mpi=True, clip_return=True):
    ...
    ddpg_params = dict()
    for name in ['max_u', 'hidden', 'layers', 'batch_size',
                 'network_class', 'polyak', 'buffer_size',
                 'Q_lr', 'pi_lr', 'relative_goals', 
                 'norm_eps', 'norm_clip', 'clip_obs',
                 'action_l2', 'scope']:
    ...
    # Update parameters
    ddpg_params.update({'input_dims': params['dims'].copy(),  # agent takes an input observations
                        'T': params['T'],
